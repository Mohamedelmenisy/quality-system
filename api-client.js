...
// api-client.js -> النسخة النهائية والمحصّنة
...
export async function getPerformanceMetrics(agentId = null) {
  try {
    ...
    if (agentId) {
      ...
      // Get the current user to check their role from metadata
      const { data: { user } } = await supabase.auth.getUser();
      userRole = user?.user_metadata?.role || 'senior'; // Default to senior if role not found in metadata
    ...
    const metrics = data[0] || {}; // Use empty object as fallback

    // --- هذا هو التحصين: توحيد شكل الـ Object اللي بيرجع ---
    const baseStructure = {
        completedOrders: 0,
        pendingEscalations: 0,
        avgResolutionTime: 0,
        totalOrders: 0,
        qualityTeamAccuracy: 'N/A',
        accuracyRate: 0.0,
        pendingReviews: 0,
        escalationRate: 0.0
    };
    ...
    if (userRole === 'manager') {
        return {
            ...
            accuracyRate: metrics.overall_team_accuracy ? parseFloat(metrics.overall_team_accuracy).toFixed(1) : 0.0,
...
// api-client.js -> >>>>>>>>>>>> هذا هو الجزء المُصحح <<<<<<<<<<<<
...
export async function getErrorTrends() {
  try {
    ...
    // This part formats the dates correctly for the chart
